...
class KaggleDatasetLoader:
    def __init__(self, data_dir: str = './Data/Text'):
        ...
        #need kaggle.json in kaggle dir before we can import the api
        ...
        if self.has_credentials:
            ...
            try:
                ...
                copy_kaggle_json(overwrite = True)
                self.api.authenticate()
    ...
    def check_kaggle_json(self) -> None:
        ...
        elif os.path.exists('Loader/kaggle.json'):
            print("kaggle.json file found in the Loader directory, copying to ~/.kaggle")
    ...
    def get_dataset(self, url: str) -> None:
        ...
        """
        assert '/' in url, 'Url provided not in valid format {user_name/dataset_name}'
        ...
        # will not overwrite a directory with files in it
        try:
    ...
    def format_dataset(self, dir_name: str, schema_name: str, delete: bool = False) -> None:
        ...
        for root, _, files in os.walk(data_pth):
            for file in tqdm(files, desc=f"Transforming files into standardized format"):
